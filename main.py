...
def main():
    ...
    for page in financial_text:
        if utils.is_cash_flows(page):
            json_data = llm_calls.extract_information(page, 'formats/cash_flows.txt')
            print(json_data)
            utils.parse_gpt(json_data, 'return_data/cash_flows.json')
        
        if utils.is_balance_sheets(page):
            json_data = llm_calls.extract_information(page, 'formats/balance_sheets.txt')
            print(json_data)
            utils.parse_gpt(json_data, 'return_data/balance_sheets.json')

        if utils.is_income_statement(page):
            json_data = llm_calls.extract_information(page, 'formats/income_statements.txt')
            print(json_data)
